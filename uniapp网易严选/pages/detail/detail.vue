<template>
	<view class="detailContainer">
		...
		<scroll-view class="content" scroll-y="true">
			<image class="detailImg" :src="goodDetail.listPicUrl" mode=""></image>
			<view class="tag" v-for="item in goodDetail.itemTagList" :key="item.tagId">{{item.name}}</view>
			<text class="price">￥ {{goodDetail.retailPrice}}</text>
			<view class="info">{{goodDetail.name}}</view>
...
<script>
	...
	import {CHANGECARTLISTMUTATION} from '../../store/mutation-type.js';
	...
	export default {
		...
		async mounted(){
			...
			let goodData=await request('/getGoodDetail',{id:goodId});
			// console.log(goodData)
			this.goodDetail=goodData.data;
			
		},
		methods:{
			...mapMutations({
				changeCartList:CHANGECARTLISTMUTATION
			}),
			addShopCart(){
				console.log('添加到购物车')
