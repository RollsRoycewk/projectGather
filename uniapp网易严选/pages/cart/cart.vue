<template>
	<view class="cartContainer">
		...
		<!-- 未登录时候的购物车 -->
		<!-- block代表块的意思,它代表内部为一个整体,但是该标签不会生成额外标签,React->Fragment -->
		...
		<block v-else>
			<block v-if="cartList.length">
				<view class="cartList">
					<view class="cartItem" v-for="(good,index) in cartList" :key="good.id">
						<text class='iconfont icon-xuanzhong' :class="good.selected?'selected':''" @click="changeSelect(!good.selected,index)"></text>
						<view class="shopItem">
							<image class="shopImg" :src="good.listPicUrl" mode=""></image>
							<view class="shopInfo">
								<text>{{good.name}}</text>
						...
						<view class="countCtrl">
							...
							<text class="count">{{good.count}}</text>
			...
			<view class="cartFooter">
				<text class='iconfont icon-xuanzhong' :class="isSelectAll?'selected':''" @click="changeSelectedAll(!isSelectAll)"></text>
				<text class="allSelected">已选 {{totalNum}}</text>
				<view class="right">
...
<script>
	...
	import {CHANGECOUNTMUTATION,CHANGESELECTMUTATION,CHANGESELECTEDALLMUTATION} from '../../store/mutation-type.js';
	export default {
		data() {
			return {
				userInfo:{}
		...
		methods:{
			...mapMutations({
				...
				changeSelectedAllMutation:CHANGESELECTEDALLMUTATION
			}),
			toLogin(){
				wx.navigateTo({
			...
			changeSelect(selected,index){
				this.changeSelectMutation({selected,index})
			},
